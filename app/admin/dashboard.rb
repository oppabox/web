ActiveAdmin.register_page "Dashboard" do
	menu :priority => 1
	content :title => "Dashboard" do
		render "dashboard"
	end

	################# get all #################
	page_action :get_all_statistic, :method => :post do

		date_from = Date.strptime(params[:date_from], "%Y-%m-%d")
		date_to = Date.strptime(params[:date_to], "%Y-%m-%d") + 1
		diff = (date_to - date_from).to_i

		date_from = "P" + date_from.strftime("%Y%m%d")
		date_to = "P" + date_to.strftime("%Y%m%d")

		if ActiveRecord::Base::connection.instance_values["config"][:adapter] == "mysql2"
			# for mysql
			query = current_admin_user.orders.paid
			.where("purchases.reference_number >= ? AND purchases.reference_number <= ?", date_from, date_to)
			.group("SUBSTRING(purchases.reference_number, 2, 8)")
			.sum("orders.quantity")
		else
			# for sqlite
			query = current_admin_user.orders.paid
			.where("purchases.reference_number >= ? AND purchases.reference_number <= ?", date_from, date_to)
			.group("SUBSTR(purchases.reference_number, 2, 8)")
			.sum("orders.quantity")
		end

		res = []
		query.keys.each do |k|
			res << [DateTime.strptime(k, "%Y%m%d").to_i * 1000, query[k]]
		end
		
		render json: { range: diff, data: res }
	end

	################# get box #################
	page_action :get_box_statistic, :method => :post do

		id = params[:id].to_i
		date_from = Date.strptime(params[:date_from], "%Y-%m-%d")
		date_to = Date.strptime(params[:date_to], "%Y-%m-%d") + 1
		diff = (date_to - date_from).to_i

		date_from = "P" + date_from.strftime("%Y%m%d")
		date_to = "P" + date_to.strftime("%Y%m%d")

		item_ids = current_admin_user.boxes.find(id).items.pluck(:id)

		if ActiveRecord::Base::connection.instance_values["config"][:adapter] == "mysql2"
			# for mysql
			query = current_admin_user.orders.paid
			.where("purchases.reference_number >= ? AND purchases.reference_number <= ? AND orders.item_id IN (?)", date_from, date_to, item_ids)
			.group("SUBSTRING(purchases.reference_number, 2, 8)")
			.sum("orders.quantity")
		else
			# for sqlite
			query = current_admin_user.orders.paid
			.where("purchases.reference_number >= ? AND purchases.reference_number <= ? AND orders.item_id IN (?)", date_from, date_to, item_ids)
			.group("SUBSTR(purchases.reference_number, 2, 8)")
			.sum("orders.quantity")
		end

		res = []
		query.keys.each do |k|
			res << [DateTime.strptime(k, "%Y%m%d").to_i * 1000, query[k]]
		end

		
		render json: { range: diff, data: res }
	end

	################# get item #################
	page_action :get_item_statistic, :method => :post do

		id = params[:id].to_i
		date_from = Date.strptime(params[:date_from], "%Y-%m-%d")
		date_to = Date.strptime(params[:date_to], "%Y-%m-%d") + 1
		diff = (date_to - date_from).to_i

		date_from = "P" + date_from.strftime("%Y%m%d")
		date_to = "P" + date_to.strftime("%Y%m%d")

		if ActiveRecord::Base::connection.instance_values["config"][:adapter] == "mysql2"
			# for mysql
			query = current_admin_user.orders.paid
			.where("purchases.reference_number >= ? AND purchases.reference_number <= ? AND orders.item_id = ?", date_from, date_to, id)
			.group("SUBSTRING(purchases.reference_number, 2, 8)")
			.sum("orders.quantity")
		else
			# for sqlite
			query = current_admin_user.orders.paid
			.where("purchases.reference_number >= ? AND purchases.reference_number <= ? AND orders.item_id = ?", date_from, date_to, id)
			.group("SUBSTR(purchases.reference_number, 2, 8)")
			.sum("orders.quantity")
		end

		res = []
		query.keys.each do |k|
			res << [DateTime.strptime(k, "%Y%m%d").to_i * 1000, query[k]]
		end
		
		render json: { range: diff, data: res }
	end
end